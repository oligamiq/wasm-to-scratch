...
pub fn rm_export_fn(module: &mut Module, rm_types: Vec<String>) -> Result<()> {
    ...
    module
        .exports
        .remove("__wasm_sb_bindgen_placeholder_anchor__")
        .map_err(|e| eyre::eyre!("{:?}", e.to_string()))?;
...
pub fn check_rm_import_fn(module: &Module) -> Result<()> {
    match module
        .imports
        .iter()
        .find(|import| import.name.starts_with("schema_version_"))
    {
    ...
    match module
        .imports
        .iter()
        .find(|import| import.name.starts_with("wasm_sb_bindgen_version_"))
    {
    ...
    match module
        .imports
        .iter()
        .find(|export| export.name.starts_with("__wasm_sb_bindgen_describe"))
    {
...
pub fn wasm_opt_module(mut module: Module) -> Result<Module> {
    ...
    // save
    module
        .emit_wasm_file(tmp.path())
        .map_err(|e| eyre::eyre!("{:?}", e.to_string()))?;

    OptimizationOptions::new_optimize_for_size_aggressively().run(tmp.path(), tmp.path())?;
    ...
    // load
    let module =
        walrus::Module::from_file(tmp.path()).map_err(|e| eyre::eyre!("{:?}", e.to_string()))?;
